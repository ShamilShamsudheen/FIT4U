...
module.exports = {
  ...
  userBlock: async (req, res) => {
    try {
      ...
      if (user.isBlocked) {
        ...
        res.status(200).json({ status: true, message: 'Trainer Blocked Successfully' })
      } else {
        res.status(200).json({ status: false, message: 'Trainer Block Cancelled' })
      }
    } catch (error) {
      console.log(error.message)
    }
  ...
  getTotalPurchaseAmount: async (req, res) => {
    try {
      const result = await Purchase.aggregate([
        {
          $group: {
            _id: null,
            totalAmount: { $sum: '$purchase_amount' }
          }
        }
      ]);
      ...
      if (result.length > 0) {
        ...
        res.status(404).json({
          error: 'No purchase records found.'
        });
      }
    } catch (error) {
      console.error('Error:', error);
      res.status(500).json({
        error: 'An error occurred while calculating total purchase amount.'
      });
    }
  },
  dashboard: async (req, res) => {
    try {
      ...
      const unBlockedCount = await User.countDocuments({ isBlocked: false });

      const approvedCount = await Trainer.countDocuments({ isApproved: true });
      const nonApprovedCount = await Trainer.countDocuments({ isApproved: false });

      res.status(200).json({
        blocked: blockedCount,
        nonBlocked: unBlockedCount,
        approved: approvedCount,
        nonApproved: nonApprovedCount,
      });
    } catch (error) {
      console.error('Error:', error);
      res.status(500).json({
